...
;;;; fastest-strstr.run.tests.lisp

(in-package #:cl-user)

(defpackage fastest-strstr.run.tests
  (:use :cl
        :fastest-strstr.lib.stub
        :af.lib.ansi-colors
        :af.lib.coverage
        :af.lib.testy)
  (:export :main))

(in-package #:fastest-strstr.run.tests)

(defparameter *base-directory* (asdf:system-source-directory :fastest-strstr))
    ...
    (if (suite
         ...
         (desc
          ...
          (it "Should echo the input"
              (eq 3 (fastest-strstr.lib.stub:echo 3)))
          )
         ) ;; end suite
...
(defun coverage ()
  ...
  (af.lib.coverage:with-coverage :fastest-strstr
    (test)
    (terpri)
...
;;; "fastest-strstr.run.tests" goes here. Hacks and glory await!
